...
use App\Orchid\Screens\MainDashboardScreen;
...
Route::screen('/main', MainDashboardScreen::class)
    ->name('platform.main');
...
Route::screen('categories', CategoryListScreen::class)
    ...
    ->breadcrumbs(fn (Trail $trail) => $trail
        ->parent('platform.index')
        ->push('카테고리 관리', route('platform.categories')));
...
Route::screen('categories/create', CategoryEditScreen::class)
    ...
    ->breadcrumbs(fn (Trail $trail) => $trail
        ->parent('platform.categories')
        ->push('카테고리 생성', route('platform.categories.create')));

Route::screen('products', ProductListScreen::class)
    ->name('platform.products')
    ->breadcrumbs(fn (Trail $trail) => $trail
        ->parent('platform.index')
        ->push('상품 관리', route('platform.products')));
...
Route::screen('products/{product}/edit', ProductEditScreen::class)
    ...
    ->breadcrumbs(fn (Trail $trail, $product) => $trail
        ...
        ->push($product->name, route('platform.products.edit', $product)));

Route::screen('products/create', ProductEditScreen::class)
    ->name('platform.products.create')
    ->breadcrumbs(fn (Trail $trail) => $trail
        ->parent('platform.products')
        ->push('상품 생성', route('platform.products.create')));

Route::screen('orders', OrderListScreen::class)
    ->name('platform.orders')
    ->breadcrumbs(fn (Trail $trail) => $trail
        ->parent('platform.index')
        ->push('주문 관리', route('platform.orders')));
...
Route::screen('orders/{order}/edit', OrderEditScreen::class)
    ...
    ->breadcrumbs(fn (Trail $trail, $order) => $trail
        ...
        ->push('주문 수정', route('platform.orders.edit', $order)));
